describe('queryLongTermMemory', () => {
 ...
 test('ignores surrounding whitespace when matching queries', () => {
  ...
  test('does not return unrelated memories for missing or empty queries', () => {
    const now = 1_700_000_000_000;
    jest.spyOn(Date, 'now').mockReturnValue(now);

    store.egoLongTermMemory = JSON.stringify([
      { text: 'diskussion ueber musik', ts: now, topics: ['kunst'], hits: 0 }
    ]);

    require('../ltmManager');

    expect(global.queryLongTermMemory('astronomie', 1)).toEqual([]);
    expect(global.queryLongTermMemory('   ', 1)).toEqual([]);
  });
...
test('upgrades legacy string entries into structured objects', () => {
  ...
  test('deduplicates repeated conversation captures and updates topics', () => {
    const now = 1_700_000_100_000;
    jest.spyOn(Date, 'now').mockReturnValue(now);

    require('../ltmManager');

    global.localStorage.setItem('egoMemoryTopics', JSON.stringify(['erstes']));
    global.localStorage.setItem('egoConversation', JSON.stringify([{ user: 'gleiche notiz', reply: 'ok' }]));
    global.localStorage.setItem('egoMemoryTopics', JSON.stringify(['zweites']));
    global.localStorage.setItem('egoConversation', JSON.stringify([{ user: 'gleiche notiz', reply: 'ok' }]));

    const parsed = JSON.parse(store.egoLongTermMemory);
    expect(parsed).toHaveLength(1);
    expect(parsed[0].text).toBe('gleiche notiz');
    expect(parsed[0].topics).toEqual(['erstes', 'zweites']);
  });
});
