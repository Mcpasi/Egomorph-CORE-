/**
 * Egomorph Core Writer
 * ===============
 * Ein lokaler Editor mit einfachen Agenten-Aktionen. Die KI-Funktionen sind
 * bewusst nur im Full-, API- oder Codex-Profil aktiv; Speichern, Export und Zaehlen
...
(function () {
  ...
  function renderShell() {
    state.root.innerHTML = [
      ...
      '    <h2 id="writerTitleText" class="writer-title">Egomorph Core Writer</h2>',
      '    <span id="writerModeText" class="writer-mode"></span>',
      '  </div>',
      '  <div class="writer-body">',
      '    <div id="writerLockNotice" class="writer-locked" hidden>Der Writer-Agent arbeitet nur im Full-, API- oder Codex-Modus. Lokales Schreiben, Speichern, Export und Wortzaehlung bleiben verfuegbar.</div>',
  ...
  function updateTranslations() {
    setText('writerTitleText', 'writerTitle', 'Egomorph Core Writer');
    setText('writerLockNotice', 'writerLockNotice', 'Der Writer-Agent arbeitet nur im Full-, API- oder Codex-Modus. Lokales Schreiben, Speichern, Export und Wortzaehlung bleiben verfuegbar.');
  ...
  function canUseAgent() {
    ...
    return profile === 'api' || profile === 'full' || profile === 'codex';
  }
  ...
  function updateAvailability() {
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      buttons[i].title = allowed
        ...
        : t('writerAgentLockedTitle', 'Nur im Full-, API- oder Codex-Modus verfuegbar');
    }
  }
  ...
  function updateModeLabel() {
    ...
    else if (profile === 'codex') el.textContent = t('writerCodexModeActive', 'Codex-Agent aktiv');
  ...
  async function runAgentAction(action) {
    if (!canUseAgent()) {
      setStatus(t('writerAgentOnlyStatus', 'Agent nur im Full-, API- oder Codex-Modus verfuegbar'), 'error');
      updateAvailability();
      return;
    }
  ...
  async function askWriterAgent(action, text, title) {
    ...
    if (profile === 'api' || profile === 'codex') {
      if (!window.egoProfile || typeof window.egoProfile.apiChatCompletion !== 'function') {
        throw new Error(t('writerApiNotReady', 'API-/Codex-Modus ist nicht bereit'));
      }
      return window.egoProfile.apiChatCompletion([
        {
          role: 'system',
          content: t('writerSystemPrompt', 'Du bist Egomorph Core Writer, ein praeziser deutscher Schreibagent. Antworte nur mit dem gewuenschten Text, ohne Vorrede.')
        },
    ...
    if (profile === 'full') {
      ...
      return window.generateWithLLM(prompt);
    }

    throw new Error(t('writerAgentUnavailable', 'Writer-Agent nur in Full, API oder Codex nutzbar'));
  }
